...
@interface PBWFullscreenPackageView : BSUICAPackageView
@end

@implementation PBWFullscreenPackageView
- (CGSize)sizeThatFits:(CGSize)size {
    CGSize bounds = self.bounds.size;
    return bounds.width > 0.0 && bounds.height > 0.0 ? bounds : [super sizeThatFits:size];
}
@end
